...
class ComboboxDelegate(QStyledItemDelegate):
    ...
    def setModelData(
        ...
        model.setData(
            index, editor.itemData(editor.currentIndex(), Qt.EditRole), Qt.EditRole
        )
...
class QNeoSelector(QWidget):
    def __init__(self, parent=None, state: ViewerState = None):
        ...
        def minus_button_clicked():
            sel = [
                self.selectedTreeModel.itemFromIndex(x).data()
                for x in self.selectedTreeView.selectedIndexes()
            ]
            for s in sel:
                self.output.analogsignals = [
                    x for x in self.output.analogsignals if id(x) != id(s)
                ]
    ...
    def get_selection(self):
        rownos = [
            self.model.itemFromIndex(x).data() for x in self.treeView.selectedIndexes()
        ]
